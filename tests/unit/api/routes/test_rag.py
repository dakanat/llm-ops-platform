...
class TestRAGQueryRoute:
    ...
    async def test_returns_200_with_valid_query(self, client: AsyncClient) -> None:
        """有効なクエリで 200 が返ること。"""
    ...
    async def test_returns_answer_from_pipeline(self, client: AsyncClient) -> None:
        """パイプラインの回答がレスポンスに含まれること。"""
        pipeline = _mock_pipeline(result=_make_generation_result(answer="RAG is a technique."))
        _override_dependencies(pipeline=pipeline)
        response = await client.post("/rag/query", json={"query": "What is RAG?"})
    ...
    async def test_returns_usage(self, client: AsyncClient) -> None:
    ...
    async def test_passes_top_k_to_pipeline(self, client: AsyncClient) -> None:
    ...
    async def test_passes_document_id_to_pipeline(self, client: AsyncClient) -> None:
    ...
    async def test_defaults_top_k_to_5(self, client: AsyncClient) -> None:
    ...
    async def test_returns_422_for_missing_query(self, client: AsyncClient) -> None:
    ...
    async def test_returns_422_for_empty_query(self, client: AsyncClient) -> None:
    ...
    async def test_returns_502_when_pipeline_fails(self, client: AsyncClient) -> None:
